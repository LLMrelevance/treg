...
For a prepared, sanitized message, use `treg feedback other - < feedback.txt`.
See `treg feedback --help` for syntax.
...
to the report. GET `{BASE}/feedback/{feedback_id}` with the same team's authentication retrieves it.
